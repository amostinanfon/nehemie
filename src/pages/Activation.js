...
const onSubmit = async (values, actions) =>{
    ...
    actions.resetForm();
        ...
        const postUsers = async() => {
            try {
                const res = await axios.post("http://82.180.137.232/api/products", values)
                console.log(res);
            } catch(err) {
                console.log(err);
            };
...
//const [file , setFile ] = useState(values.file);
  ...
  return (    
    <form onSubmit={handleSubmit} autoComplete='off'>
        <label style={{color: 'red', fontSize:'20px'}}>ACTIVATION DE L'ID</label>
        ...
        <label htmlFor='img'>Capture d'écran</label>
        <input 
            value={values.img}
            accept='image/*'
            onChange={handleChange}
